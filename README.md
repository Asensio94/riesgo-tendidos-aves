...
[mapas y rankings por región](https://asensio94.github.io/riesgo-tendidos-aves/regiones.html). Se regenera el día 2 de cada mes
con GitHub Actions. La presentación estática vive en `web/` y las miniaturas se generan con `herramientas/miniaturas.py`.

## Uso

```bash
pip install -r requirements.txt
python -m riesgo.cli run --region estrecho
```
...
| Fichero | Contenido |
|---|---|
| `informe.html` | informe con mapa embebido, tabla estacional por especie y top de elementos |
| `mapa.html` | mapa interactivo (folium): capas mensuales, por mecanismo y por especie, infraestructura y ranking |
| `ranking_elementos.csv` / `.geojson` | todos los apoyos, aerogeneradores y tramos con índice máximo, medio, mes pico y especies |
| `riesgo_total.tif`, `riesgo_electro.tif`, `riesgo_col_lin.tif`, `riesgo_col_aer.tif` | rásteres de 12 bandas (una por mes), EPSG:4326, para QGIS |
| `topografia.tif` | elevación, pendiente y TPI |

Opciones útiles:

```bash
python -m riesgo.cli regiones                      # regiones predefinidas (estrecho, tarifa, cantabria, monfrague, gallocanta)
python -m riesgo.cli run --region cantabria        # una región puede llevar su propia lista de especies (config.REGIONES[...]["especies"])
python -m riesgo.cli especies                      # especies y pesos
python -m riesgo.cli run --bbox 39.6,-6.3,40.0,-5.7 -e "Aegypius monachus" -e "Ciconia nigra"
python -m riesgo.cli run --region estrecho --movebank        # añade datasets publicados en Movebank (descargas de cientos de MB)
python -m riesgo.cli run --region estrecho --study-id 123456 # estudios Movebank concretos (públicos o con MOVEBANK_USER/MOVEBANK_PASSWORD)
python -m riesgo.cli indice                                  # portada output/index.html con todas las regiones (se regenera en cada run)
python -m riesgo.cli puntuar --region cantabria --observatorio ../observatorio-alegaciones   # índice de riesgo de cada proyecto en información pública
python -m riesgo.cli puntuar --region cantabria --geojson proyectos.geojson                  # o de cualquier polígono
```

`puntuar` devuelve, por proyecto, el índice medio y máximo, el mes pico, el porcentaje de celdas con infraestructura y el
máximo por mecanismo (`output/<region>/proyectos_puntuados.csv`). Con `--observatorio` toma los anuncios geolocalizados del
...
`data/ebirdst/<Genus_species>/` y `riesgo/ebirdst.py` los reproyecta a la malla, agrega las 52 semanas en meses y los
mezcla con la frecuencia GBIF (70/30, `PESO_EBIRDST`). La especie pasa a confianza 1. Sin ficheros, no cambia nada.
...
- Los pesos por especie y el peso de conservación están en `riesgo/config.py` (síntesis de Bevanger 1998, Janss 2000,
...
3. `abundance.py`: citas GBIF descargadas por bloques anuales (de reciente a antiguo, hasta `GBIF_MAX_CITAS` = 20 000 por
...
8. `puntuar.py`: estadísticas del índice sobre polígonos de proyectos (integración con el observatorio de alegaciones).
...
- En el observatorio de alegaciones, mostrar el índice de `puntuar` junto a cada anuncio de eólica o red eléctrica.
